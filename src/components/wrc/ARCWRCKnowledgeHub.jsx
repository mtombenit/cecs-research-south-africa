          ...
          <div style={s.card}>
            ...
            <div style={{flex:"1 1 220px",minWidth:"200px"}}>
              {!selectedPair ? (
                ...
                const catA = Object.entries(D.cat_counts).find(([,]) => {
                  return D.sites.some(site => site.cats.includes(Object.keys(CAT_COLORS).find(k => k === selectedPair.a || selectedPair.a.includes(k.split(" ")[0]))));
                });
                const colorA = Object.entries(CAT_COLORS).find(([k])=>selectedPair.a.toLowerCase().includes(k.split(" ")[0].toLowerCase()))?.[1] || "#2563EB";
                const colorB = Object.entries(CAT_COLORS).find(([k])=>selectedPair.b.toLowerCase().includes(k.split(" ")[0].toLowerCase()))?.[1] || "#7C3AED";
                const sharedSites = D.sites.filter(site => {
                  const names = D.top_conc.map(c=>c.name);
                  return true;
                });
                // Find sites that detected both (approximated from co-occurrence count)
                const topSites = D.sites.filter(s=>s.n_contaminants>=3).slice(0,selectedPair.v);
                const strengthLabel = selectedPair.v >= 14 ? "Very Strong" : selectedPair.v >= 8 ? "Strong" : selectedPair.v >= 4 ? "Moderate" : "Weak";
                const strengthColor = selectedPair.v >= 14 ? "#DC2626" : selectedPair.v >= 8 ? "#D97706" : selectedPair.v >= 4 ? "#2563EB" : "#64748B";
                const catAName = D.top_conc.find(c=>c.name===selectedPair.a)?.cat || "";
                const catBName = D.top_conc.find(c=>c.name===selectedPair.b)?.cat || "";
                const concA = D.top_conc.find(c=>c.name===selectedPair.a);
                const concB = D.top_conc.find(c=>c.name===selectedPair.b);
                return (
                  <div style={{background:"#FFFFFF",border:"1px solid #E2E8F0",borderRadius:"8px",padding:"16px",boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"}}>
                      <div style={{fontSize:"0.62rem",color:"#64748B",letterSpacing:"0.1em",textTransform:"uppercase",fontWeight:"600"}}>Pair Analysis</div>
                      <button onClick={()=>setSelectedPair(null)} style={{background:"none",border:"none",cursor:"pointer",color:"#94A3B8",fontSize:"0.9rem",lineHeight:1}}>✕</button>
                    </div>

                    {/* Compound names */}
                    <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"12px",flexWrap:"wrap"}}>
                      <span style={{background:colorA+"18",color:colorA,border:`1px solid ${colorA}44`,borderRadius:"4px",padding:"3px 8px",fontSize:"0.65rem",fontWeight:"700"}}>{selectedPair.a}</span>
                      <span style={{color:"#94A3B8",fontSize:"0.7rem"}}>+</span>
                      <span style={{background:colorB+"18",color:colorB,border:`1px solid ${colorB}44`,borderRadius:"4px",padding:"3px 8px",fontSize:"0.65rem",fontWeight:"700"}}>{selectedPair.b}</span>
                    </div>

                    {/* Co-occurrence count */}
                    <div style={{background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:"6px",padding:"10px 12px",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div>
                        <div style={{fontSize:"1.6rem",fontWeight:"700",color:"#2563EB",lineHeight:1}}>{selectedPair.v}</div>
                        <div style={{fontSize:"0.6rem",color:"#64748B",marginTop:"2px"}}>shared sampling sites</div>
                      </div>
                      <div style={{textAlign:"right"}}>
                        <div style={{fontSize:"0.68rem",fontWeight:"700",color:strengthColor}}>{strengthLabel}</div>
                    ...
                    {(catAName || catBName) && (
                      <div style={{marginBottom:"12px"}}>
                        <div style={{fontSize:"0.6rem",color:"#64748B",letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:"600",marginBottom:"6px"}}>CEC Categories</div>
                        {catAName && <div style={{fontSize:"0.65rem",color:"#475569",marginBottom:"3px"}}>• <strong>{selectedPair.a}</strong>: {catAName}</div>}
                        {catBName && catBName !== catAName && <div style={{fontSize:"0.65rem",color:"#475569",marginBottom:"3px"}}>• <strong>{selectedPair.b}</strong>: {catBName}</div>}
                        {catBName && catBName === catAName && <div style={{fontSize:"0.65rem",color:"#059669",fontStyle:"italic"}}>Both from same category</div>}
                      </div>
                    )}
