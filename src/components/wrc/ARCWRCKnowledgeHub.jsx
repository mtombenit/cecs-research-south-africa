...
const s = {
  ...
  header: { background:"linear-gradient(135deg, #0D9B6E 0%, #14B87F 60%, #0D9B6E 100%)", borderBottom:"3px solid #34D399", padding:"24px 28px 0" },
  title: { fontSize:"1.4rem", fontWeight:"700", color:"#FFFFFF", letterSpacing:"0.03em", marginBottom:"4px" },
  subtitle: { fontSize:"0.68rem", color:"#6EE7B7", letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"20px" },
      ...
      <div style={s.body}>
          ...
          <>
            ...
            <div style={s.grid2}>
              ...
              <div style={s.card}>
                ...
                {["KwaZulu-Natal dominates (253 records, 31%) driven by ARV and microplastic estuarine studies — eThekwini hosts SA's largest WWTP network and the Durban/uMgeni estuarine system, making it the most intensively studied province.","Northern Cape has ZERO records across all 9 CEC categories — this represents a critical monitoring void, not absence of contamination; the Orange River and major irrigation schemes remain entirely uncharacterised.","PPCPs account for 47% of all records, reflecting SA's dual disease burden: antibiotics (Clarithromycin, Sulfamethoxazole) dominate alongside ARVs, both discharged via the same WWTP effluent pathway.","ARVs (78 records) are detected exclusively from KZN WWTP influent/effluent — South Africa has the world's largest ART programme (~5.5 million people on treatment), making ARV excretion a nationally significant contamination source.","Efavirenz at 34,000 ng/L in WWTP influent is the highest detected ARV. DEWATS (Decentralised Wastewater Treatment System) shows 0% removal — Efavirenz is highly lipophilic (log Kow = 5.0) and resistant to conventional activated sludge treatment.","Lopinavir concentrations INCREASE through all three KZN WWTPs (−52% to −192%). This negative removal is caused by desorption from sewage solids: Lopinavir (log Kow = 6.9) sorbs strongly to sludge during treatment and is then released to the effluent stream."].map((t,i)=>(
        ...
        {tab === "Co-occurrence" && (
          <div style={s.card}>
                    ...
                    return (
                      <tr key={row}>
                        ...
                        {D.top_names.map(col=>{
                          ...
                          return <td key={col} onClick={()=>v>0&&setSelectedPair({a:row,b:col,v})} style={{background:bg,width:"24px",height:"22px",textAlign:"center",color:v>0?(intensity>0.5?"#FFFFFF":"#1E3A5E"):"#CBD5E1",fontSize:"0.55rem",border:"1px solid #E2E8F0",cursor:v>0?"pointer":"default",transition:"opacity 0.1s"}} title={`${row} + ${col}: ${v} shared sites`}>{v>0?v:""}</td>;
                        })}
                      </tr>
                    );
                  })}
            ...
            <div style={s.insight}>▸ <strong>Nevirapine + Efavirenz (17 sites)</strong> is the highest co-occurrence: both are NNRTIs (non-nucleoside reverse transcriptase inhibitors) used together in SA's standard first-line ART regimen (previously Atripla; now TLD-based), so they are co-excreted by millions of patients into the same WWTP influent. ▸ <strong>Clarithromycin + Carbamazepine (16 sites)</strong>: both are highly persistent in the aquatic environment (half-lives &gt;100 days in surface water) with similar hydrophilicity, leading to shared transport along river catchments. ▸ <strong>Microplastic cluster</strong> (Fragments–Films–Fibres–Polystyrene, 7–14 shared sites) reflects the same KZN estuarine sampling campaigns where all polymer types co-occur in marine-coastal sediments.</div>
          </div>
        )}
          ...
          <>
            <div style={s.grid2}>
              ...
              <div style={s.card}>
                ...
                {[
                  { source:"WWTP Effluent", color:"#3B82F6", signature:"PPCPs + ARVs + Microbial CECs", detail:"Conventional activated sludge WWTPs remove hydrophilic drugs poorly. SA's dual disease burden (HIV/ART + opportunistic infection prophylaxis) means WWTP influent in KZN carries Efavirenz (34,000 ng/L), Nevirapine (2,800 ng/L) and Sulfamethoxazole (6,968 ng/L) — all confirmed by 78 ARV records exclusively from WWTP matrices." },
                  { source:"Industrial/Mining", color:"#6B7280", signature:"Heavy Metals + PAHs", detail:"Mpumalanga's 24 heavy metal records cluster around Limpopo geothermal springs and gold mine drainage. PAHs in Limpopo (Mokolo River) trace to coal combustion at Medupi Power Station. Free State gold mine water shows Cu, Pb, Zn from Witwatersrand-era tailings leaching." },
                  { source:"Agricultural Runoff", color:"#D97706", signature:"Pesticides + Alkylphenols", detail:"Lourens River (Western Cape, 6 pesticide compounds) receives runoff from Helderberg fruit farms. Vaal River APEOs (22 records) originate from livestock dips and surfactant use. North West's 22 APEO records at Rietspruit Weir confirm nonylphenol ethoxylate inputs from agricultural washing operations." },
                  { source:"Marine/Estuarine", color:"#0891B2", signature:"Microplastics (Fragments + Films)", detail:"All 56 marine/coastal microplastic records are from KZN estuaries (uMgeni, Isipingo, Mdloti, Durban Bay). PE/PP/PS polymer suite reflects urban stormwater inputs from informal settlements in the eThekwini Metro, where &gt;1 million people lack formal waste collection services." },
                  { source:"Urban Stormwater", color:"#DB2777", signature:"APEOs + PPCPs + Pesticides", detail:"Jukskei River (Gauteng, 8 compounds across 2 categories) is the clearest multi-source urban fingerprint in the dataset — it drains Alexandra and Midrand urban catchments, receiving both domestic PPCP discharge from informal settlements and pesticide runoff from Midrand horticultural areas." },
                ].map(src=>(
