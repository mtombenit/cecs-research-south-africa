...
export default function PendingPapersMonitor() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.PendingPaper.delete(id),
    onSuccess: () => {
  ...
  const clearAllMutation = useMutation({
    mutationFn: async (ids) => {
      for (const id of ids) {
        await base44.entities.PendingPaper.delete(id);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pending-papers'] });
      toast.success("Cleared all errors");
    },
  });
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => clearAllMutation.mutate(duplicatePapers.map(p => p.id))}
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => clearAllMutation.mutate(rejectedPapers.map(p => p.id))}
                disabled={clearAllMutation.isPending}
          ...
          <div className="p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button
                size="sm"
                variant="ghost"
                onClick={() => clearAllMutation.mutate(errorPapers.map(p => p.id))}
                disabled={clearAllMutation.isPending}
                className="text-red-700 hover:text-red-800 hover:bg-red-100"
              >
                <Trash2 className="w-3 h-3 mr-1" />
